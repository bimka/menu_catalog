...
class SubmenuDB:
    ...
    def get_submenu_by_title(self, title):
        return self.db.query(Submenu).filter(Submenu.title == title).first()

    def get_submenu_by_id(self, submenu_id: uuid.UUID):
        return self.db.query(Submenu).filter(Submenu.id == submenu_id).first()
    ...
    def update_submenu(self, submenu_id: uuid.UUID, submenu: dict):
        if submenu.get("title"):
            ...
            self.db.query(Submenu).filter(Submenu.id == submenu_id).update(
                {"title": new_title}
            )
