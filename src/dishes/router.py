...
from src.dishes import schemas, service
...
@dish_router.get("/", response_model=list[schemas.Dish])
...
def delete_dish(dish_id: uuid.UUID, db: Session = Depends(get_db)):
    db_dish_status = service.delete_dish(dish_id, db)
...
@dish_router.patch("/{dish_id}", response_model=schemas.Dish)
